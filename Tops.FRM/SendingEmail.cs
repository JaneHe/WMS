    ...
    public class SendingEmail
    {
        ...
        public void T_Elapsed(object sender, ElapsedEventArgs e)
        {
            try
            {
                ...
                if (dt0.Hour >= 8 && dt0.Hour <= 18)
                {
                    ...
                    int count = new Tops.FRM.DbHelperSQL(0).ExecuteSql("up_UpdateOverTimeEmail", rownamesp1);//发送后修改状态为已发送
                }
        ...
        public void SendResetPasswordEmail()
        {
            ...
            try
            {
                ...
                DataTable dt1 = SelInWorkProduct("up_sendingEmail", rownamesp1);
                //生成Excel表格
                if (dt1.Rows.Count == 0)
                {
                    return;
                ...
                else
                {
                    ...
                    mail.To.Add(consigeenEmailAddre);//收件人邮箱
                    mail.To.Add(consigeenEmailAddre2);
                    mail.Priority = MailPriority.High;  //优先级别最高   
                    mail.Subject = DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd") + " 超时数据";  //主题
                    mail.Body = "超时数据";
