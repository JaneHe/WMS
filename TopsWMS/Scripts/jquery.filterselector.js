...
(function($){
	...
	$.filterSelector.prototype = {
		...
		options: {
			...
			style: '',
			checkAllText: '全选 / 取消全选',
			noneSelectedText: '选择物料类型',
			selectedText: '# 种材料被选中',
		...
		init: function(options, el){
			...
			this.input = $('<input ' + (o.filter !== true ? 'readonly="readonly"' : '') + ' ></input>')
		...
		bindEvents: function(){
			...
			if(o.filter === true){
				input.bind({
					...
					focus: function(){
					    selector.hasClass("selector-disabled") || (input.toggleClass('selector-input-active', true), input.attr("readonly", "readonly"), input.val(""), self.open());
					}
				});
			}else{
				input.bind({
