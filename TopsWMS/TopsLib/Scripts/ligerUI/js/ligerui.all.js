...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [50, 100, 200, 1000, 2000,5000],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
        ...
        */
        ...
        endEdit: function (rowParm)
        {
            ...
            else if (rowParm != undefined)
            {
                ...
                if (rowdata==null||(!g.editors[rowdata['__id']])) return;
        ...
        */
            ...
            $.each(o, function (i) {
                if (o[i] instanceof Date) {
                    ...
                    $.each(g.columns, function () {
                        if (this.columnname == i) {
                            column = this; return false;
                        }
                    });
                    o[i] = p.formatters['date'].call(g, o[i], column);
                }
            });
            //***********************************************//
        ...
        _initColumns: function ()
        {
            ...
            $(g.columns).each(function (i, column)
            {
                ...
                column.dateFormat = column.dateFormat || "yyyy-MM-dd";
        ...
        _getCellHtml: function (rowdata, column)
        {
            var g = this, p = this.options;
            if (column.isrownumber)
                return '<div class="l-grid-row-cell-inner"  ' + (p.fixedCellHeight?(' style = "height:' + p.rowHeight + 'px;" '):'') + ' >' + (parseInt(rowdata['__index']) + 1) + '</div>';
        ...
        {
            var g = this, p = this.options;
            if ($.trim(p.serverTotal) != '') {
                var footerData = g.data[p.serverTotal];
                var footerColVal = '';
                if (footerData)
                    footerColVal = footerData[column.columnname] == null ? "" : footerData[column.columnname];
                return "<div>" + footerColVal + "</div>";
            }
            var totalsummaryArr = [];
        ...
        {
            var g = this, p = this.options;
            if (!g.isTotalSummary() && $.trim(p.serverTotal) == '') return false;
        ...
        _noSelection: function () {
            ...
            //                    //防止（火狐和webkit）拖动时选择文本
            $("body").css("-moz-user-select", "none")
            $("body").css("-khtml-user-select", "none");
            $("body").css("user-select", "none");
        },
        _canSelection: function () {
            document.onselectstart = function () { return true; }
            $("body").css("-moz-user-select", "").css("-khtml-user-select", "");
        },
        _setEvent: function ()
        {
            var g = this, p = this.options;
        ...
        selectAll: function () {
            var g = this, p = this.options;
            var rowParms = g.rows;
            if (rowParms && rowParms.length > 0) {
                $.each(rowParms, function () {
                    var rowid = this['__id'];
                    var rowobj = g.getRowObj(rowid);
                    var rowobj1 = g.getRowObj(rowid, true);
                    if (rowobj) $(rowobj).addClass("l-selected");
        ...
        _onResize: function ()
        {
            ...
            if (p.height && p.height != 'auto')
            {
                ...
                //g._setHeight(h);
                g._setHeight(h > p.minHeight ? h : p.minHeight);
            }
            if (g.enabledFrozen())
            {
...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        expandedAll: function () {
            var g = this, p = this.options;
            $("li[isexpand='true']>.l-body>.l-expandable-close", g.tree).click();
        },
