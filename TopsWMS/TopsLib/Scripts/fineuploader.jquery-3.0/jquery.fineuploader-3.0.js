...
qq.FineUploaderBasic = function(o){
    ...
    this._options = {
        ...
        messages: {
            ...
            //typeError2: "{file} has an invalid extension.",
            //sizeError: "{file} is too large, maximum file size is {sizeLimit}.",
            //minSizeError: "{file} is too small, minimum file size is {minSizeLimit}.",
            //emptyError: "{file} is empty, please select files again without it.",
            //noFilesError: "No files to upload.",
            //onLeave: "The files are being uploaded, if you leave now the upload will be cancelled."
            typeError: "{file} 不能上传. 可上传的类型有: {extensions}.",
            typeError2: "{file} 不能上传",
            sizeError: "{file} 文件超过指定大小： {sizeLimit}.",
            minSizeError: "{file} is too small, minimum file size is {minSizeLimit}.",
            emptyError: "{file} 是空文件，请重新选择",
            noFilesError: "No files to upload.",
            onLeave: "文件正在上传中，假如现在离开，将取消上传"
        },
        retry: {
            enableAuto: false,
            maxAutoAttempts: 3,
...
qq.FineUploaderBasic.prototype = {
    ...
    _validateFile: function(file){
        ...
        if (!this._isNoAllowedExtension(name)) {
            this._error('typeError2', name);
            return false;
    ...
    _isNoAllowedExtension: function (fileName) {
        var ext = (-1 !== fileName.indexOf('.'))
            ? fileName.replace(/.*[.]/, '').toLowerCase()
            : '';
        var notAllowed = this._options.validation.noAllowedExtensions;
